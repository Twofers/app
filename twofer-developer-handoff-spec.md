...
6. **Email confirmation flow. DECIDED and SUPABASE CONFIG VERIFIED 2026-06-13: keep email confirmation.** The live Supabase dashboard has custom SMTP enabled through Resend (`smtp.resend.com`) from `support@twoferapp.com`, Confirm email is ON, Site URL is `https://www.twoferapp.com`, the four app redirect URLs are allow-listed, the auth email send limit is 30/hour, the per-user SMTP interval is 30 seconds, and the confirm-signup template uses `{{ .ConfirmationURL }}`. DNS has DKIM, DMARC, and the `send.twoferapp.com` SPF/MX return-path records. Do not expose SMTP credentials. The client handles confirmed, unconfirmed, and existing-account signup paths (`app/auth-landing.tsx`, `lib/auth-password-recovery.ts`, `app/auth-callback.tsx`), including resend confirmation and the correct unconfirmed-login message. On 2026-06-13, `unvmex2@hotmail.com` was confirmed to be an existing, already-confirmed Supabase user created on 2026-03-26, so a "new account" attempt with that address should not be expected to receive a fresh confirmation email.
...
**Resolved dashboard item, still needs fresh-address inbox QA.** Email confirmation remains ON. On 2026-06-13 the live Supabase project was verified with Resend custom SMTP, the app redirect allow-list, auth email rate limits, and a confirm-signup template that uses `{{ .ConfirmationURL }}`. Supabase accepted `/signup` and `/resend` requests during testing. The `unvmex2@hotmail.com` report was traced to an already-confirmed existing user, not a missing new-user auth row. Because Supabase intentionally avoids disclosing registered addresses during signup, the auth screen copy now tells users to sign in if the email already has a Twofer account. Before store review, still run a fresh throwaway-address inbox test, resend test, on-device confirmation-link tap, and password-reset link tap.
...
Before any store submission, also run the final fresh-address email confirmation QA from section 17 and resolve the open items in section 4.
