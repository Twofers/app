...
`,
  "20260723120000_offer_versions_foundation.sql": readRequired(path.join("docs", "dev", "ai_deal_studio_dev_business_locations.sql")),
};

PRELUDE_BY_MIGRATION["20260723120000_offer_versions_foundation.sql"] += `\n${readRequired(path.join("docs", "dev", "ai_deal_studio_dev_redemptions_minimal.sql"))}`;
PRELUDE_BY_MIGRATION["20260723120000_offer_versions_foundation.sql"] += `\n${readRequired(path.join("docs", "dev", "ai_deal_studio_dev_offer_backfill_columns.sql"))}`;
...
function normalizeSupabaseDevSql(sql) {
  return stripPaidSurfaceComments(sql)
    .replace(/\buuid_generate_v4\(\)/gi, "gen_random_uuid()")
    .replace(/CREATE\s+POLICY\s+("[^"]+"|[A-Za-z_][A-Za-z0-9_]*)\s+ON\s+([^\s]+)\s+/gi, (match, policyName, tableName) => {
      return `DROP POLICY IF EXISTS ${policyName} ON ${tableName};\n${match}`;
    })
    .replace(/ALTER\s+TABLE\s+([^\s]+)\s+ADD\s+CONSTRAINT\s+("[^"]+"|[A-Za-z_][A-Za-z0-9_]*)\s+/gi, (match, tableName, constraintName) => {
...
for (const fileName of SELECTED_MIGRATIONS) {
  ...
  const sql = normalizeSupabaseDevSql(`${PRELUDE_BY_MIGRATION[fileName] ?? ""}\n${readRequired(fullPath)}`);
  assertNoBlockedContent(fileName, sql);
  chunks.push(`\n\n-- ============================================================\n-- Source migration: ${fileName}\n-- ============================================================\n\n${sql}\n`);
}

for (const filePath of APPENDED_SQL) {
  const sql = normalizeSupabaseDevSql(readRequired(filePath));
