...
serve(async (req) => {
  ...
  try {
    ...
    // Demo account: return hardcoded translations
    ...
    if (isDemoUserEmail(user.email) && !demoWantsLive) {
      ...
      const demoResult: TranslationResult = {
        title_es: title ? `${title} (oferta especial)` : "",
        title_ko: title ? `${title} (특별 할인)` : "",
        description_es: description ? `${description} \u00a1No te lo pierdas!` : "",
        description_ko: description ? `${description} 놓치지 마세요!` : "",
      };
