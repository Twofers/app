...
function paidCancelingStatus(status: string | null): "pro_canceling" | "paid_canceling" | null {
  if (status === "pro_active" || status === "pro_canceling") return "pro_canceling";
  if (status === "paid_active" || status === "paid_canceling") return "paid_canceling";
  return null;
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (config.purchaseSurface !== "in_app_link") {
      return jsonResponse(
        req,
        { error: "Paid subscription cancellation is not enabled.", error_code: "PURCHASE_SURFACE_DISABLED" },
        403,
      );
    }
    ...
    const { data: entitlement, error: entitlementError } = await supabaseAdmin
      ...
      .select(
        "billing_account_id,status,first_paid_at,current_period_ends_at,cancel_at_period_end,provider_subscription_id,billing_accounts(provider_customer_id)",
      )
    ...
    const nextStatus = paidCancelingStatus(status);
    if (!nextStatus) {
      return jsonResponse(req, { error: "This location does not have an active paid subscription to cancel." }, 409);
    }
    if ((status === "pro_canceling" || status === "paid_canceling") && entitlement?.cancel_at_period_end === true) {
      return jsonResponse(req, {
        status: nextStatus,
        current_period_ends_at: entitlement.current_period_ends_at ?? null,
      });
    }
    if (status !== "pro_active" && status !== "paid_active") {
      return jsonResponse(req, { error: "This subscription is already canceling." }, 409);
    }
    ...
    if (!billingAccountId || !subscriptionId || !stripeCustomerId || !entitlement?.first_paid_at) {
      return jsonResponse(req, { error: "Missing Stripe subscription details for this paid subscription." }, 400);
    }
    ...
    if (subscription?.deleted === true || subscription?.status !== "active") {
      return jsonResponse(req, { error: "Stripe subscription is not active." }, 409);
    }
    ...
    const periodEndsAt = unixSecondsToIso(updated.current_period_end) ?? entitlement?.current_period_ends_at ?? null;
    ...
    console.error("[stripe-cancel-paid-subscription] error:", err);
    return jsonResponse(req, { error: "Failed to cancel paid subscription." }, 500);
  }
});
