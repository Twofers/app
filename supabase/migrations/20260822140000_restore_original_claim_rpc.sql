-- FULL REVERT of 20260822120000 / 20260822130000: restore
-- claim_approved_business_application_for_user to its 20260817120000 definition,
-- byte for byte.
--
-- Why: 20260822120000 added "#variable_conflict use_column" to clear a real
-- 42702 ("column reference business_id is ambiguous") that blocks an approved
-- merchant from ever claiming a workspace. On the test project that fix worked.
-- In production it made get-business-onboarding-context return HTTP 500
-- deterministically (4 of 4 calls) for a merchant whose application is
-- trial_active -- an endpoint that returned 200 before. Reverting the separate
-- trial_active widening in 20260822130000 did NOT clear the 500, which isolates
-- the pragma as the cause: it changes ambiguous resolution for the WHOLE
-- function body, and production data reaches a path where some reference
-- legitimately means the variable rather than the column.
--
-- Restoring known-good behaviour first. That reinstates two known bugs:
--   * 42702 blocks first-time workspace claim for newly approved merchants
--   * trial_active owners still see the approval_required dashboard (F-21)
-- Neither has live users today, and both are strictly better than a hard 500 on
-- a merchant-facing endpoint.
--
-- The real fix must qualify the specific ambiguous sites (the two
-- ON CONFLICT (business_id) targets and the INSERT column lists) or rename the
-- OUT parameters, rather than flipping resolution globally -- and must be
-- validated against BOTH claim shapes: materialize-new-business AND
-- link-existing-business, which is the shape production actually has and the
-- one my test pass never exercised.
