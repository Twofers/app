-- ROLLBACK of the trial_active widening in 20260822120000, keeping its 42702 fix.
--
-- 20260822120000 did two things. The #variable_conflict pragma fixed a real P0:
-- an approved merchant could not claim a workspace at all because
-- "column reference business_id is ambiguous" aborted the materialize path.
-- That fix is correct and is KEPT here.
--
-- The second change let the fresh-claim branch also match status =
-- 'trial_active', so a comped or already-paying owner could claim. Applying it
-- to production regressed get-business-onboarding-context from 200 to 500 for
-- an affected merchant, and reproducing the shape on the test project showed
-- why it must not ship as written: when the application already points at an
-- existing business, the claim path runs
--   UPDATE public.businesses SET status = 'approved_not_activated', ...
-- which DOWNGRADES a live trialing business. On the test project a business
-- seeded as trialing came out of the claim as approved_not_activated. Production
-- escaped the downgrade only because its transaction errored and rolled back.
--
-- So the lockout (F-21) stays open and is safer than this cure. Fixing it
-- properly means the claim must not rewrite business/subscription state for a
-- business that is already activated -- a larger change that needs its own
-- design and test pass, not a predicate widening.
--
-- Net effect of this migration: keep the onboarding fix, drop the widening.
