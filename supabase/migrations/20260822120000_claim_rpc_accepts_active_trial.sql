-- Fix: a comped or already-paying merchant could be permanently locked out of
-- the app with "Business setup opens after your application is approved."
--
-- get-business-onboarding-context deliberately never trusts a pre-existing
-- owner/member row; it resolves the caller's business ONLY through
-- claim_approved_business_application_for_user. That RPC matched either
--   (a) an application already carrying claimed_by_user_id (any status), or
--   (b) an unclaimed application whose status is exactly 'approved_not_activated'.
-- claimed_by_user_id is written in exactly one place: inside this RPC.
--
-- So an application that reaches 'trial_active' BEFORE its owner first signs in
-- satisfies neither branch. The RPC returns no row, the context returns
-- business:null, and the merchant sees a fully disabled dashboard forever.
--
-- That is reachable through the normal launch motion, not just legacy data:
-- stripe-webhook sets trial_active on checkout, and the admin comp grant does
-- too. stripe-create-checkout-session already had to widen its own gate for
-- exactly this reason -- see its comment: "Applying an admin grant moves the
-- application to trial_active ... so restricting this to approved_not_activated
-- would leave every comped owner unable to convert." This applies the same
-- widening to the claim RPC, which never received it.
--
-- Both production businesses were observed in the broken end state on
-- 2026-07-24 (businesses.status = trialing, application status = trial_active,
-- claimed_by_user_id NULL).
--
-- Scope: only the fresh-claim match widens. The already-claimed branch and the
-- owner-conflict guard (v_existing_owner_id IS DISTINCT FROM p_user_id -> RETURN)
-- are untouched, so this changes WHEN a legitimate owner may claim, never WHO.
-- The claim does not write business_applications.status, so an active trial is
-- not downgraded; access_tier is now preserved for trial_active applications
-- instead of being reset to approved_not_activated.
--
-- After applying: node scripts/probe-rls-smoke.mjs
--                 node scripts/probe-merchant-surfaces.mjs
--                 confirm get-business-onboarding-context returns a real
--                 business + true capabilities for an affected merchant.
...
-- PL/pgSQL variables, and the body's INSERT column lists and ON CONFLICT
-- (business_id) targets name those same columns. Postgres cannot tell which is
-- meant and raises 42702 "column reference business_id is ambiguous", which
-- aborted the claim the moment it tried to materialize a workspace.
--
-- Verified on the test project 2026-07-24: a freshly approved, unclaimed
-- application (status approved_not_activated, no business yet) -- the ordinary
-- onboarding path -- failed with 42702, so no approved merchant could claim a
-- workspace at all. This is pre-existing and independent of the trial_active
-- widening below.
--
-- Safe here because every ambiguous site in this body is an INSERT column list
-- or an ON CONFLICT target that means the column; the OUT parameters are only
-- ever assigned (:=) at the very end, which this pragma does not affect.
...
BEGIN
  ...
  WHERE COALESCE(ba.approved_email_normalized, lower(btrim(ba.email))) = v_email
    AND ba.status IN ('approved_not_activated', 'trial_active')
  ...
  UPDATE public.business_applications
    SET claimed_by_user_id = p_user_id,
        ...
        access_tier = CASE
          WHEN v_application.status = 'trial_active' THEN COALESCE(v_application.access_tier, 'approved_not_activated')
...
COMMENT ON FUNCTION public.claim_approved_business_application_for_user(uuid, text) IS
  'Service-role only, race-safe claim of exactly one unclaimed approved_not_activated OR trial_active application for a confirmed auth email (trial_active included so comped/already-paying owners can still claim on first sign-in). Atomically materializes/links an inert setup workspace, profile, membership, billing shell, and trial-eligible location without trial dates, credits, access, or Stripe objects.';
