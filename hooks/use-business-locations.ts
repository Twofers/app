...
export function maxLocationsForTier(tier: SubscriptionTier): number {
  if (!isMobilePaidBillingEnabled()) return 1;
...
export function useBusinessLocations(businessId: string | null, subscriptionTier: SubscriptionTier) {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      // Locations are keyed by `businesses(id)` — the same id deals.location_id
      // resolves through and the id this hook receives from useBusiness().
