...
- Billing surfaces are enabled in code with `PAID_BILLING_ENABLED = true`, and `PILOT_DISABLE_BILLING_GATE = false` — the pilot bypass was turned OFF on 2026-07-06 once billing enforcement was verified, so the gate is live. (Corrected 2026-07-24; this line previously said `true`, which contradicted `lib/billing/access.ts`.) Separately, in-app mobile billing UI is disabled (`isMobilePaidBillingEnabled()` returns `false`) and the four `billing*` screens are deliberate redirect stubs, so activation happens on the website and there is no in-app checkout path.
