(() => {
  ...
  const reportsEndpoint = Shell.endpoint("admin-reports");
  const nameRequestsEndpoint = Shell.endpoint("admin-business-name-requests");
  ...
  async function adminPostTo(endpoint, body) {
    return Shell.adminPost(endpoint, body);
  }
