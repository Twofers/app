(() => {
  ...
  // --- shell helpers (run against whichever header is currently live: the
  // minimal signed-out shell, or the injected dashboard fragment). Audit
  // F-015: the dashboard markup ships in /admin/app.html and is fetched only
  // when a stored admin token exists, so the /admin landing page renders no
  // internal IA signed out. NOTE: app.html itself is still a public static
  // file (recon-only; every data call requires a server-verified session);
  // true server-side gating needs Vercel middleware — Dan-gated follow-up.
  ...
  function hasStoredToken() {
    return Boolean(sessionStorageSource().getItem(tokenKey));
  }
  ...
  function wireSignOut() {
    const signOutButton = document.querySelector("[data-admin-sign-out]");
    if (!signOutButton || signOutButton.dataset.wired) return;
    signOutButton.dataset.wired = "true";
  ...
  // --- dashboard (runs only after /admin/app.html is injected) --------------

  function initDashboard(configEl) {
    const endpoint = configEl.dataset.adminSummaryEndpoint;
    const authEndpoint = configEl.dataset.adminAuthEndpoint;
    const aiUsageEndpoint = configEl.dataset.adminAiUsageEndpoint;
  ...
  async function boot() {
    wireSignOut();
    syncNavForSession();
    if (!hasStoredToken()) return; // signed out: the minimal shell stays as-is.

    const root = document.querySelector("[data-admin-app-root]");
    if (!root) return; // not the /admin entry shell (e.g. section pages).
    ...
    root.innerHTML = markup; // fragment is markup-only; injected scripts never run.
    const configEl = root.querySelector("[data-admin-app]");
    if (!configEl) {
      setStatus("Could not load admin summary", "danger");
      return;
    }
    wireSignOut(); // the injected header carries its own sign-out button.
    initDashboard(configEl);
  }
