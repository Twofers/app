...
| Claim flow (10.5) | `app/(tabs)/index.tsx:514-583`, `app/deal/[id].tsx:248-290`, `supabase/functions/claim-deal/index.ts:407-600` | One active claim app-wide (idempotent same-deal), business repeat policy (redemption-based; replaced the old per-business-per-local-day rule — see `docs/claim-rules.md`), max-claims, unique token + 6-char short code with collision retry, redeem-by reminder scheduled. |
