...
18. **Email confirmation app-side (17, item 6)** — signup → `emailRedirectTo` → `auth-callback` → awaiting-verification banner is coherent (`app/auth-landing.tsx:359-372,495-521`). Gaps: **no `supabase.auth.resend()` anywhere** (lost email = stuck user), and "Email not confirmed" maps to the wrong-password message (`lib/i18n/api-messages.ts:94-96`) / surfaces raw English via `friendlyAuthMessage`. Supabase-side SMTP/toggle/allow-list remain Dan's manual task per item 6.
