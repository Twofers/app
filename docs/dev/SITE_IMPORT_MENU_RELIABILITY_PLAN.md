...
The exact Gemini cause is **not diagnosable from here**: the provider's error message is deliberately not persisted (`error_message='unknown'`), and reproducing the transform offline is inconclusive (the stripped schema looks valid) — nailing it needs the live Gemini error, which requires either a temporary instrumented deploy (Dan-gated) or the prod `GEMINI_API_KEY` to replay against Google. Tracked as a separate follow-up; **not** on the critical path, because the fix below routes around it.
