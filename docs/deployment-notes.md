...
`ai-generate-deal-copy`, `ai-deal-suggestions`, and `ai-translate-deal` also return plain-language errors with `error_code: OPENAI_NOT_CONFIGURED` when `OPENAI_API_KEY` is missing. `ai-generate-deal-copy` can continue through the shared Gemini text router only when the router flags and `GEMINI_API_KEY` are configured.
