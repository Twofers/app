...
| `ai-generate-ad-variants` | `lib/functions.ts` | `business_id`, `hint_text`, `business_context`, locale and optional media/revision fields | `{ ad, quota? }` | Yes for AI-first owner flow | None in wrapper (throws) | If unavailable, owner ad generation blocks primary path. |
