...
- `ai-generate-ad-variants` logs OpenAI/Gemini image generation and edit failures with sanitized status/error codes only; raw upstream image-provider response bodies are not stored in console logs or `ai_generation_costs`.
