...
- `ai-compose-offer` returns `OPENAI_KEY_MISSING` when `OPENAI_API_KEY` is missing, preventing canned compose output or voice transcripts from appearing as live AI results. Upstream Whisper transcription failures return `TRANSCRIPTION_FAILED` without raw provider response bodies.
