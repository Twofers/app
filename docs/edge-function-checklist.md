...
| `ai-create-deal` | No current app caller | Any POST body | HTTP 410 with `AI_CREATE_DEAL_LEGACY_DISABLED` | No (legacy disabled) | None | Permanently disabled; no provider call, signed URL, Supabase client, or live `deals` insert path remains in the function source. |
...
- `ai-business-lookup` throws to callers when lookup fails (`lib/functions.ts`), so UI shows error banners instead of hardcoded Irving data. Google lookup exception paths log fixed error codes, not raw exception text.
- `ai-extract-menu` keeps synthetic menu samples behind `AI_EXTRACT_MENU_ALLOW_SAMPLE_WITHOUT_KEY=true`; production-style missing provider config returns `OPENAI_NOT_CONFIGURED`, and upstream provider/outer handler failures log sanitized status or fixed error details rather than raw provider bodies or exception text.
- `ai-generate-ad-variants` logs research, copy, image QA, image generation, and image edit failures with sanitized stage/error codes only; raw upstream image-provider response bodies and free-form exception text are not stored in console logs, `ai_generation_costs`, or copy-failure rows.
- `ai-compose-offer` returns `OPENAI_KEY_MISSING` when required provider configuration is missing, preventing canned compose output or voice transcripts from appearing as live AI results. Text/photo compose uses the shared provider router and can continue through Gemini only when the router flags and `GEMINI_API_KEY` are configured. Upstream Whisper transcription failures return `TRANSCRIPTION_FAILED`, and Whisper/live compose provider/config/outer handler failures log only sanitized status or generic failure details.
- `ai-generate-deal-copy` throws in `lib/functions.ts`, and the Edge function returns `OPENAI_NOT_CONFIGURED` when `OPENAI_API_KEY` is missing and no configured Gemini router path can continue, preventing fake AI copy. It uses the shared text provider router and returns `AI_GENERATION_FAILED`/`AI_TEXT_CONFIG_INVALID` without raw provider response bodies or exception text.
- `ai-deal-suggestions` returns `OPENAI_NOT_CONFIGURED` when `OPENAI_API_KEY` is missing and no configured Gemini router path can continue, preventing canned insight cards from appearing as live AI recommendations. It uses the shared text provider router and returns sanitized generation/config errors without raw provider response bodies or exception text.
- `ai-translate-deal` returns `OPENAI_NOT_CONFIGURED` when `OPENAI_API_KEY` is missing and no configured Gemini router path can continue, preventing deterministic phrase-table translations from being saved as AI translations. It uses the shared text provider router and returns sanitized generation/config/server errors without raw provider response bodies or exception text.
