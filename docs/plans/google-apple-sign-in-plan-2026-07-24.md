...
- [x] New Google user, customer role selected → **PASS (Phase 3b, after Dan deleted the test auth user).** Chose Shopper on the finish-setup step → routed to the customer profile setup (ZIP + optional birthday, consistent with the locked decisions), "Signed in as sandersrodgers79@gmail.com". Cold restart returned to profile setup rather than finish-setup, proving `profiles.role = customer` persisted.
...
- [x] Existing **confirmed** email/password user, same email, taps Google → **PASS, confirmed in the dashboard 2026-07-25.** `unvmex2@gmail.com` is a **single** user with providers `Email, Google` and `Created at: Sat 13 Jun` — Supabase attached the Google identity to the pre-existing password account rather than creating a duplicate, and the stored role was preserved (routed as a customer). No error copy needed.
- [ ] Existing **unconfirmed** email/password user, same email, taps Google → **NOT TESTED** (no unconfirmed account available).
- [x] Google-created user later tries password login → **PASS (Phase 3b).** `sandersrodgers79@gmail.com` (Google-only, no password) + a wrong password → "Email or password is incorrect." Localized via `friendlyAuthError`, no crash, and no disclosure that the account uses Google. Correct failure mode; no new copy needed.
- [x] Brand-new user taps Google from the **login** tab → finish-setup state appears, terms enforced, role persists. **PASS (Phase 3b, 2026-07-25.)** With the auth user deleted, a login-tab tap produced `LOG [auth] social_finish_setup — sandersrodgers79@gmail.com` and rendered exactly the inline step: "Choose your account type" + both role cards + terms checkbox + Continue, with the email/password form and the login/signup tabs correctly hidden. Continue is inert until terms are accepted (same disabled-until-terms rule as the signup submit). **This run also verifies the F-2 fix on device: zero `PROFILES_ROLE_IMMUTABLE` warnings on the exact path that previously raced** — before the fix the derived `customer` write fired here.
- [ ] ~~**NOT TESTED — the case never arose.**~~ (superseded by the PASS above; the earlier note was correct at the time — no tap had been a brand-new account *without* a pending role) (Corrected 2026-07-25; an earlier note here blamed the F-2 race for making it unreachable, which was wrong.) `social_finish_setup` needs a brand-new account with **no** pending role, and none of the four taps was that: tap 1 was a new account *with* pending=business (`adopt`, correct), tap 2 the same account with a stored role (`route`, correct), tap 3 was cancelled, tap 4 was `unvmex2@gmail.com` which is a pre-existing user (`route`, correct). Every branch taken was the right one; this branch simply had no input that would select it. Needs a Google account that has never signed in to Twofer.
...
**F-4 — RETRACTED, not a defect.** During Phase 3b I flagged the finish-setup Continue button as inconsistent for being `disabled` until terms are accepted rather than enabled-with-an-error. That was wrong: the existing signup submit is `disabled={busy || submitBlockedByTerms}` with the same 0.6 opacity (`app/auth-landing.tsx:1222`), so the new button matches the established pattern exactly. The comment I cited ("submit stays enabled … surfaces inline errors") sits on `validateFields` and governs email/password field errors, not the terms gate. No change made.
...
- [x] **Flip `EXPO_PUBLIC_ENABLE_SOCIAL_AUTH` to `"true"` in the `production` profile of `eas.json`** — done 2026-07-25 on Dan's instruction, after the Android matrix closed green. `apk` inherits it via `extends`. The flag is read at bundle time, so it takes effect only in the next EAS production build (still gated).
- [ ] **Before that build: confirm Supabase → Authentication → Providers → Google lists BOTH client IDs** (web *and* iOS, comma-separated). On iOS the id token's `aud` is the iOS client ID; on Android it is the web one. If only the web ID is authorized, **Android keeps working and iOS fails** with an audience error — and because the Android matrix is fully green, that asymmetry reads as an iOS code bug rather than a config gap. The `/auth/v1/settings` endpoint only exposes whether Google is enabled, not which client IDs, so this cannot be checked from the CLI.
